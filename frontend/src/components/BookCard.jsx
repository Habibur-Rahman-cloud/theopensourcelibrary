...
const BookCard = ({ book, index, onView }) => {
    const handleClick = (e) => {
        trackBookClick(book);
        if (onView) {
            // If onView is provided, we might be using the modal 
            // but for SEO we still want the Link to work.
            // If you want to ONLY open modal, you'd use e.preventDefault()
        }
    };
    ...
    return (
        ...
        >
            <Link to={`/book/${book.slug}`} onClick={handleClick}>
                {/* Book Cover */}
